...
/// Get streaming response with range request support
/// Implements HTTP range requests for efficient video streaming
...
fn main() {
  ...
  tauri::Builder::default()
    .setup(|app| {
      ...
      // Create 2x2 grid of streaming webviews
      // Each webview is isolated with its own context

      // Top-left: Primary stream
      let _webview1 = window.add_child(
        tauri::webview::WebviewBuilder::new("stream1", WebviewUrl::App(Default::default()))
          .auto_resize(),
        LogicalPosition::new(0., 0.),
        LogicalSize::new(width / 2., height / 2.),
      )?;

      // Top-right: Secondary stream
      let _webview2 = window.add_child(
        tauri::webview::WebviewBuilder::new("stream2", WebviewUrl::App(Default::default()))
          .auto_resize(),
        LogicalPosition::new(width / 2., 0.),
        LogicalSize::new(width / 2., height / 2.),
      )?;

      // Bottom-left: Tertiary stream
      let _webview3 = window.add_child(
        tauri::webview::WebviewBuilder::new("stream3", WebviewUrl::App(Default::default()))
          .auto_resize(),
        LogicalPosition::new(0., height / 2.),
        LogicalSize::new(width / 2., height / 2.),
      )?;

      // Bottom-right: Quaternary stream
      let _webview4 = window.add_child(
        tauri::webview::WebviewBuilder::new("stream4", WebviewUrl::App(Default::default()))
          .auto_resize(),
    ...
    .run(tauri::generate_context!(
      "../../examples/streaming-hub/tauri.conf.json"
    ))
